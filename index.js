        ...
        <center>
            ...
            <footer class="footer">
                <div id="music">
                    ...
                    <br>Thanks For Using <b>Fca-Shadow-Remake</b> - From <b>Kanzu</b> <3<br>
                </div>
            </footer>
            </div>
        </center>
...
try {
    ...
    mainPromise = utils
        ...
        Fetch('https://raw.githubusercontent.com/blackbotdzcuto/Fca-Shadow-Remake/main/package.json').then(async (/** @type {{ body: { toString: () => string; }; }} */res) => {
            const localVersion = JSON.parse(readFileSync('./node_modules/fca-shadow-remake/package.json')).version;
                if (Number(localVersion.replace(/\./g,"")) < Number(JSON.parse(res.body.toString()).version.replace(/\./g,"")) ) {
                    log.warn("[ FCA-Shadow ] •",getText(Language.NewVersionFound,JSON.parse(readFileSync('./node_modules/fca-shadow-remake/package.json')).version,JSON.parse(res.body.toString()).version));
                    if (global.Fca.Require.FastConfig.AutoUpdate == true) { 
                        log.warn("[ FCA-Shadow ] •",Language.AutoUpdate);
                            try {
                                execSync('npm install fca-shadow-remake@latest', { stdio: 'inherit' });
...
function setUserNameAndPassWord() {
    ...
    let localbrand2 = JSON.parse(readFileSync('./node_modules/fca-shadow-remake/package.json')).version;
    console.clear();
    console.log(figlet.textSync('Shadow', {font: 'ANSI Shadow',horizontalLayout: 'default',verticalLayout: 'default',width: 0,whitespaceBreak: true }));
